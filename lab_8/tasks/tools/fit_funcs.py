...
def least_sq(xy):
    ...
    if len(x_cor)== len(y_cor):
        ...
        delta = N * np.sum(np.square(x_cor)) - np.sum(x_cor)**2

        A = (N * np.sum(x_cor*y_cor) - np.sum(x_cor)*np.sum(y_cor))/delta
        
        B = (np.sum(np.square(x_cor)) * np.sum(y_cor) - np.sum(x_cor) * np.sum(x_cor*y_cor))/delta
